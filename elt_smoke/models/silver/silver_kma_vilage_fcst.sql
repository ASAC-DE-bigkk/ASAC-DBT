with bronze as (
    ...
    from {{ source('ask_seoul_bronze', 'kma_vilage_fcst') }}
),

standardized as (
    select
        *,
        cast(date_parse(concat(base_date, base_time), '%Y%m%d%H%i') as timestamp(6)) as issued_at,
        cast(date_parse(concat(fcst_date, fcst_time), '%Y%m%d%H%i') as timestamp(6)) as forecast_at
    from bronze
