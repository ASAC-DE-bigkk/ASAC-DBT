...
```powershell
python -m pytest -q contracts tests domains/traffic/contracts/tests domains/weather/contracts/tests --ignore=dbt_packages --disable-warnings -p no:cacheprovider
git diff --check HEAD
```
