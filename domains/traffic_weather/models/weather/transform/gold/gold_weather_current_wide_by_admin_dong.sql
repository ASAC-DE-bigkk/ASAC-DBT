...
target_slice as (
    select src.admin_dong_code, min(src.forecast_at) as forecast_at
    from src
    cross join kst_now
    where src.forecast_at >= kst_now.current_hour_at
    group by src.admin_dong_code
),

sliced as (
    select src.*
    from src
    inner join target_slice
        on src.admin_dong_code = target_slice.admin_dong_code
       and src.forecast_at = target_slice.forecast_at
),
