-- Weather expected-slot coverage is derived only from the materialized state Gold.
...
with state as (
    select
        ...
        cast(
            {{ asac_axes.utc_to_kst('collection_slot_at') }}
            as timestamp(6)
        ) as issued_at_kst,
    ...
    from {{ ref('gold_weather_collection_slot_state') }}
),
...
aggregated as (
    ...
    from classified
    group by source_id, issued_at_kst
),
