...
def _legacy_script(domain: str, filename: str) -> Path:
    return PROJECT_ROOT / "contracts" / domain / "scripts" / filename
...
def test_contract_engine_is_an_explicit_importable_package() -> None:
    package_markers = (
        PROJECT_ROOT / "contracts" / "__init__.py",
...
def test_monoproject_ci_scope_tracks_both_domains_and_shared_contract_engine() -> None:
    ...
    workflow = workflow_module.read_text(encoding="utf-8")
    assert '"domains/traffic_weather/"' in workflow
    assert '"packages/asac_axes/"' in workflow
    assert '".github/workflows/traffic-weather-monoproject-premerge-gate.yml"' in workflow
