...
- dbt project: `domains/traffic_weather` (`dbt_project.yml`, `profiles.yml`)
...
python contracts/traffic/scripts/validate_singular_test_dependency_manifest.py \
...
`domains/traffic_weather` 또는 `packages/asac_axes`가 바뀌면 다음 검증을
`domains/traffic_weather` project root에서 실행한다.

1. project `dbt deps`
2. isolated target의 `dbt parse --no-partial-parse`
3. 모든 named selector의 non-empty 검증
4. fresh manifest를 입력으로 받는 project 전체 Python 계약 테스트
5. 생성된 manifest의 Traffic singular-test dependency 검증

```bash
python -m workflows.premerge_gate run \
  --repository-root ../.. \
  --project-dir . \
...
domain workflow Module은 변경 범위 판정부터 전체 gate 순서를 소유한다. `dbt deps` 뒤에
synthetic `traffic_snapshot_dag_run_id=ci__traffic-weather-monoproject-premerge-gate`를 사용해
새 target에서 `dbt parse --no-partial-parse`를 수행하고, 생성된 `manifest.json`을 Python
계약 테스트와 기존 singular-test dependency validator가 공유한다. gate가 실패해도 target artifact는
`if: always()`로 수집하므로 배포 전 graph 오류의 원인을 확인할 수 있다.

이 project와 공용 `asac_axes` package에 무관한 PR만 skip step을 거쳐 성공 종료한다.
