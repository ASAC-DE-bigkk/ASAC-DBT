...
transit 의 장기 이력은 이 아카이브 gold 가 유일하다. 사용자향 gold(선정 9종,
[2026-07-15-gold-candidates-user-facing.md](2026-07-15-gold-candidates-user-facing.md))는 전부 여기서 파생된다.
...
- `gold_transit_dong_hourly`(#67)의 15분판. 구조(3원 outer 결합) 동일, 추가 컬럼 2계열:
  - **`bus_*_t1`**: tier1(간선·광역) 한정 버스 지표. 시간대끼리 비교하는 파생(G4 리듬, G10 예측)은
    반드시 이것만 소비 — tier2 는 07·13·19시에만 관측돼 시간대별 표본 구성이 달라진다(#440).
    전 티어 `bus_*` 는 현재 상태·커버리지(420개 동) 용도.
...
## dim_transit_bus_route_tier — 노선 티어 분류 (관측 역산)

- routeType 원천(R2 reference json)이 웨어하우스에 없어, **최근 7일 관측 시간대 종류 수**로 역산:
  tier1(매 30분·전 시간대)은 ~19종, tier2(07·13·19시만)는 ≤3종 → 임계 12
  (티어링 배포 과도기의 간헐 전 노선 수집이 윈도에 남아도 오분류하지 않는 마진 — 모델 주석).
- 매 빌드 전체 재계산(table), 기준 시각은 `max(event_at)`(수집 중단에도 분류 유지).
- reference 를 웨어하우스로 적재하는 정공법이 생기면 그 조인으로 대체 가능.
