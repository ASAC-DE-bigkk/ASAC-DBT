...
| [normalization-plan.md](normalization-plan.md) | **정규화 검토(제안, 미적용)** — detail payload 저카디널리티 컬럼 실측 분류 + 테이블 폭증 방지 설계안 |
| [partitioning-indexing-plan.md](partitioning-indexing-plan.md) | **파티셔닝·인덱싱 검토(제안, 미적용)** — 85테이블 인덱스 실태(전부 PK뿐) 실측·EXPLAIN 근거·우선순위 |
