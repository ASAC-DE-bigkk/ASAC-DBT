...
def test_snapshot_reconciliation_independently_derives_state_and_evidence():
    ...
    assert (
        "nullif(trim(coalesce(failure_reason, '')), '') is not null"
        in compact_sql
    )
    ...
    for evidence_column in (
        ...
        assert f"gold.{evidence_column} is distinct from expected.{evidence_column}" in compact_sql
...
def test_snapshot_reconciliation_keeps_one_current_evidence_row_when_current_is_empty():
    ...
    assert "count_if(current_rows.dag_run_id is distinct from configured_run.dag_run_id)" in current_evidence_sql
    assert "count_if(current_rows.source_id is distinct from configured_run.source_id)" in current_evidence_sql
