...
INCIDENT_SOURCE_ID = "source.traffic.traffic_bronze.seoul_traffic_incident"
AUDIT_SOURCE_ID = (
...
def _parse_manifest(
    ...
    *,
    ...
    sandbox_root = tmp_path / f"dbt-root-{case_name}"
    project_root = sandbox_root / "domains" / "traffic"
    package_root = sandbox_root / "packages" / "asac_axes"
    target_path = tmp_path / f"target-{case_name}"
    log_path = tmp_path / f"logs-{case_name}"
    environment = os.environ.copy()

    shutil.copytree(TRAFFIC_ROOT, project_root)
    shutil.copytree(
        TRAFFIC_ROOT.parents[1] / "packages" / "asac_axes",
        package_root,
    )
