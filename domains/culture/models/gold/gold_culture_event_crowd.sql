-- gold(크로스도메인): 문화 행사가 열리는 자치구의 시간대별 평시 혼잡 베이스라인(#276).
--   그레인: gu_code × hour_of_day. citydata gold(장소×시간 인구혼잡)를 source로 읽어
--   event_schedule 이 있는 gu 로 스코프. "그 행사 지역 저녁 몇 시가 붐비나/한적한가".
...
-- (gu, hour) × 레벨 빈도 → 모달 혼잡 레벨
lvl_counts as (
    select c.gu_code, c.hour_of_day, c.area_congest_lvl, count(*) as cnt
    from crowd c
    join event_gu e on e.gu_code = c.gu_code
    group by c.gu_code, c.hour_of_day, c.area_congest_lvl
),
typical as (
    select gu_code, hour_of_day, max_by(area_congest_lvl, cnt) as typical_congest
    from lvl_counts
...
agg as (
    ...
    from crowd c
    join event_gu e on e.gu_code = c.gu_code
    group by c.gu_code, c.hour_of_day
)
...
join typical t on t.gu_code = a.gu_code and t.hour_of_day = a.hour_of_day
