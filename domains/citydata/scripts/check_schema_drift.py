#!/usr/bin/env python3
"""citydata schema.yml ↔ 실제 테이블 컬럼 드리프트 검사 (CI 게이트).

SQL(비즈니스 로직)이 산출하는 실제 컬럼과 schema.yml(문서/컨텍스트) 선언이 어긋나면 실패한다.
- **yml 누락**: 테이블엔 있는데 schema.yml 에 없는 컬럼 (문서화 안 됨) → FAIL
- **yml 유령**: schema.yml 엔 있는데 테이블엔 없는 컬럼 (오래된 문서) → FAIL
- **설명 없음**: schema.yml 에 있으나 description 없는 컬럼 → 기본 WARN (--strict 면 FAIL)
...
DEFAULT_YML = os.path.normpath(os.path.join(HERE, "..", "models", "schema.yml"))
...
def main() -> int:
    ...
    doc = yaml.safe_load(open(args.yml, encoding="utf-8"))
    models = {m["name"]: m for m in (doc.get("models") or [])}
    ...
    if fail:
        print("❌ 드리프트 발견 — schema.yml 과 실제 컬럼을 맞추세요.")
        return 1
    print("✅ 드리프트 없음.")
    return 0
